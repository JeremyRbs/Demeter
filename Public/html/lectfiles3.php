...
$query = "SELECT HeureDispo, NomClient,  GROUP_CONCAT( NomProd) as NomProd, commande.NumCom  from Detail JOIN com_det ON detail.Num_OF = com_det.Num_OF JOIN commande ON commande.NumCom = com_det.NumCom WHERE commande.A_Livrer = 'O' AND commande.EtatLivraison = 'N' GROUP BY NomClient ORDER BY commande.HeureDispo ";// GROUP BY NomClient
